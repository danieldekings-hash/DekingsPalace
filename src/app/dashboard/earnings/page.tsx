...
import { getEarningsSummary, listEarnings, getEarningsToday, withdrawEarnings, type EarningsItem, type EarningsSummary, type EarningsToday } from '@/lib/api';
...
export default function EarningsPage() {
  ...
  const [today, setToday] = useState<EarningsToday | null>(null);
  ...
  const load = async () => {
    ...
    try {
      // Load summary, today's earnings, and list in parallel
      const [summaryData, todayData, earningsData] = await Promise.all([
        getEarningsSummary(token),
        getEarningsToday(token),
        listEarnings({
          type: filterType,
          isWithdrawn: filterWithdrawn === 'all' ? undefined : filterWithdrawn === 'true',
          page: 1,
          pageSize: 100, // Get more items for client-side search/filter
          sortBy,
          sortOrder,
        }, token)
      ]);
  ...
  const filteredItems = useMemo(() => {
    // API already handles type and isWithdrawn filtering, so we only need to filter by search term
    const filtered = items.filter(it => {
      // Filter by search term (client-side only, API doesn't support search)
    ...
    // API already handles sorting, but we keep this for client-side search results
    // (sorting is already done server-side, but if search filters results, we maintain the sort)
    filtered.sort((a, b) => {
      let comp = 0;
      switch (sortBy) {
        case 'date':
  ...
  return (
    <div className="dashboard-page container-custom earnings-page">
      ...
      {/* Summary Cards - Show skeleton while loading, then fade in content */}
      {loading ? (
        <>
        ...
        <>
          <div className="row mb-4 content-fade-in">
      ...
      {/* Earnings History - Show skeleton while loading, then fade in content */}
      <div className="mt-4">
        ...
        {loading ? (
          ...
          <div className="card border-gold card-hover content-fade-in">
            <div className="card-body p-0">
              <Table
                data={filteredItems}
                columns={columns}
